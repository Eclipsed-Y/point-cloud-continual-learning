...
class resnet_Manifold_Matching(pl.LightningModule):
    def __init__(self,args):
        ...
        self.model = self.create_model(args).to(device)
    ...
    def train(self, train_loader, val_loader, task_num = 0, ref_model = None):
        ...
        # for e in range(1):
            ...
            for x, y, task_id in train_loader:
                ...
                if ref_model is not None:
                    ...
                    old_x = self.mem_data[er_mem_indices].to(device)
                    old_y = self.mem_label[er_mem_indices].to(device)
                    old_task = self.mem_task_id[er_mem_indices].to(device)
                    #
                    ref_index = ((old_task - 1) * old_x.shape[0] + torch.Tensor(range(old_x.shape[0])).to(
                        old_x.device)).long()
                    # old_feature = F.adaptive_avg_pool2d(F.relu(self.model.features(old_x)),1)
                    ...
                    #     old_ref_feature = F.adaptive_avg_pool2d(F.relu(ref_model.features(old_x)),1)
                    #     old_ref_feature = old_ref_feature.view(old_ref_feature.size(0), -1)
            ...
            if ref_model is not None:
                print(f'<=== epoch: {e}   loss: {tot_loss / tot_size}   Accuracy: {Acc}   time: {time_cost}  similarity: {self.similarity(ref_model)}===>')
        ...
        self.model.load_state_dict(torch.load(r'checkpoint/resnet_best_parameters' + str(self.episodic_mem_size) + '.pkl'))
        ...
        for x_batch, y_batch, task_batch in dataset_loader:
            for index in range(x_batch.shape[0]):
                ...
                if self.episodic_mem_size > self.mem_used:
                    ...
                    self.mem_data[self.mem_used] = x.cpu()
                    self.mem_label[self.mem_used] = y.cpu()
                    self.mem_task_id[self.mem_used] = t.cpu()
                    self.mem_used += 1
                else:
                    # if center == None:
                    #     ######## AlexNet
                    ...
                    #     # replay_feature = torch.cat([self(self.mem_data[i * 5: (i + 1) * 5], self.mem_task_id[i * 5: (i + 1) * 5]) for i in range(0,self.episodic_mem_size // 5)], dim = 0)
                    #     replay_feature = torch.cat([self(self.mem_data[i * 10: (i + 1) * 10].cuda(), self.mem_task_id[i * 10: (i + 1) * 10].cuda()) for i in range(0,self.episodic_mem_size // 10)], dim = 0)
                    ...
                    #     self.mem_data[j], self.mem_label[j], self.mem_task_id[j] = x.cpu(), y.cpu(), t.cpu()
                    #     distance[j] = dis_to_center
                    #     sorted_index = torch.argsort(distance)
                    # else:
                    #     j = np.random.randint(0, self.data_seen)
                    #     if j < self.episodic_mem_size:
                    #         # self.mem_data[sorted_index[j]], self.mem_label[sorted_index[j]], self.mem_task_id[sorted_index[j]] = x, y, t
                    #         self.mem_data[sorted_index[j]], self.mem_label[sorted_index[j]], self.mem_task_id[sorted_index[j]] = x.cpu(), y.cpu(), t.cpu()
                    ...
                    j = np.random.randint(0, self.data_seen)
                    if j < self.episodic_mem_size:
                        self.mem_data[j], self.mem_label[j], self.mem_task_id[
                            j] = x.cpu(), y.cpu(), t.cpu()

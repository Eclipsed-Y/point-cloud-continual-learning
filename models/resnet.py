...
def conv7x7(in_planes, out_planes, stride=1):
    return nn.Conv2d(in_planes, out_planes, kernel_size=7, stride=stride, padding='same', bias=False)

class PreActBlock_BN(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, droprate=0):
        super(PreActBlock_BN, self).__init__()
    ...
    def forward(self, x, task):
        key = str(int(task[0]))
        # print(type(self.bn1))
        out = F.relu(self.bn1[key](x))
        shortcut = self.shortcut(out) if hasattr(self, 'shortcut') else x
        out = self.conv1(out)
        if self.drop is not None:
            out = self.drop(out)
        out = self.conv2(F.relu(self.bn2[key](out)))
        # 投影
        out += shortcut
        return out

class PreActResNet_BN(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, in_channels=3):
        super(PreActResNet_BN, self).__init__()
        self.in_planes = 64
    ...
    def forward(self, x,task):
        ...
        x = F.relu(self.bn_last[key](x))
        x = F.adaptive_avg_pool2d(x, 1)
        x = self.logits(x.view(x.size(0), -1),task)
        return x

    def orthogonal_weights_init(self):
    ...
    def OrthogonalRegular(self):
        diff = self.orth_dist(self.stage2[0].shortcut[0].weight) + self.orth_dist(self.stage3[0].shortcut[0].weight) + self.orth_dist(self.stage4[0].shortcut[0].weight)
        diff += self.deconv_orth_dist(self.stage1[0].conv1.weight, stride=1) + self.deconv_orth_dist(self.stage1[1].conv1.weight, stride=1)
        diff += self.deconv_orth_dist(self.stage2[0].conv1.weight, stride=2) + self.deconv_orth_dist(self.stage2[1].conv1.weight, stride=1)
        diff += self.deconv_orth_dist(self.stage3[0].conv1.weight, stride=2) + self.deconv_orth_dist(self.stage3[1].conv1.weight, stride=1)
        diff += self.deconv_orth_dist(self.stage4[0].conv1.weight, stride=2) + self.deconv_orth_dist(self.stage4[1].conv1.weight, stride=1)
        return diff

class PreActBlock_PROJ_GN(nn.Module):
    expansion = 1
    def __init__(self, in_planes, planes, stride=1, droprate=0):
        super(PreActBlock_PROJ_GN, self).__init__()
        self.stride = stride
        # self.bn1 = nn.BatchNorm2d(planes,affine = False)
        # self.bn1 = nn.GroupNorm(num_groups=2, num_channels=planes,affine=False)
        ...
        # self.bn2 = nn.GroupNorm(num_groups=2, num_channels=planes,affine=False)
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            ...
            # self.shortcut_bn = nn.BatchNorm2d(self.expansion * planes,affine = False)
            # self.shortcut_bn = nn.GroupNorm(num_groups=2, num_channels=self.expansion * planes,affine=False)
            self.shortcut_bn = nn.GroupNorm(num_groups=2, num_channels=self.expansion * planes)
            # self.shortcut = nn.Sequential(
            #     nn.Conv2d(in_planes, self.expansion*planes, kernel_size=1, stride=stride, bias=False),
            #     nn.BatchNorm2d(self.expansion * planes)
            # )
    ...
    def forward(self, x,task):
        ...
        if hasattr(self,'shortcut_conv'):
            ...
            # shortcut = self.shortcut_bn[key](shortcut)
            # shortcut = self.feature_proj(shortcut,self.proj[key])
...
class PreActResNet_PROJ_GN(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, in_channels=3):
        super(PreActResNet_PROJ_GN, self).__init__()
        self.in_planes = 64
        last_planes = 512*block.expansion
        self.last_plane = last_planes
        ...
        # self.bn1 = nn.GroupNorm(num_groups=2,num_channels=64,affine=False)
...
class PreActBlock_GN(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, droprate=0):
        super(PreActBlock_GN, self).__init__()
        self.in_planes = in_planes
        self.planes = planes
        # self.bn1 = nn.BatchNorm2d(in_planes)
...
class PreActResNet_GN(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, in_channels=3):
        super(PreActResNet_GN, self).__init__()
        self.in_planes = 64
        last_planes = 512*block.expansion
        self.last_planes = last_planes
        self.last_plane = last_planes
        ...
        self.stage4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        # self.bn_last = nn.BatchNorm2d(last_planes)
        self.bn_last = nn.GroupNorm(num_groups=2,num_channels=last_planes)
        self.last = nn.Linear(last_planes, num_classes)
...
class PreActBottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, droprate=None):
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion*planes, kernel_size=1, stride=stride, bias=False)
                # nn.BatchNorm2d = nn.BatchNorm2d(self.expansion*planes),
            )
...
class PreActResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, in_channels=3):
        ...
        self.stage4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        self.bn_last = nn.BatchNorm2d(last_planes)
        # self.bn_last = nn.GroupNorm(2,last_planes)
        self.last = nn.Linear(last_planes, num_classes)
    ...
    def features(self, x):
        # out = self.bn1(self.conv1(x))
        out = self.conv1(x)
        out = self.stage1[0](out)
        out = self.stage1[1](out)
        out = self.stage2[0](out)
        out = self.stage2[1](out)
        out = self.stage3[0](out)
        out = self.stage3[1](out)
        out = self.stage4[0](out)
        out = self.stage4[1](out)
        return out
...
def ResNet18_PROJ_GN(out_dim = 10):
    return PreActResNet_PROJ_GN(PreActBlock_PROJ_GN, [2, 2, 2, 2], num_classes=out_dim)

def ResNet18_GN(out_dim = 10):
    return PreActResNet_GN(PreActBlock_GN, [2,2,2,2], num_classes=out_dim)

def ResNet18_BN(out_dim = 10):
    return PreActResNet_BN(PreActBlock_BN, [2,2,2,2], num_classes=out_dim)